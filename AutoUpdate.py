...
class DeleteClient:
    ...
    def __init__(self, menu_instance):
        ...
        # Create the title label
        title_label = tk.Label(self.frame, text="Delete Client", font=("Arial", 15))
        title_label.grid(row=0, column=0, columnspan=2, padx=10, pady=10)


            # Create a button to upload a ZIP file
        self.select_site = tk.Button(self.frame, text="Select Site", command=self.select_site, bg="green", fg="white", width=12)
        self.select_site.grid(row=1, column=0, columnspan=2, padx=10, pady=10, sticky="w")



        # Create a button to create directories
        self.create_button = tk.Button(self.frame, text="Delete", command=self.create_directories, bg="blue", fg="white", width=12)
        self.create_button.grid(row=7, column=0, columnspan=2, padx=10, pady=10, sticky="w")


        # Label to display the selected ZIP file
        self.selected_zip_label = tk.Label(self.frame, text="")
        self.selected_zip_label.grid(row=6, column=0, padx=10, pady=10)
        ...
        self.checkbox_vars = []  # Initialize the list here
    ...
    def create_directories(self):
        try:
            # Get the selected site directory
            base_dir = self.selected_zip_label.cget("text")

            # Get the paths of selected subdirectories
            subdirs = [os.path.join(base_dir, subdir) for var, subdir in zip(self.checkbox_vars, os.listdir(base_dir)) if var.get()]
            ...
            for subdir in subdirs:
                pool = self.locate_appPpool(subdir)
                #variable_name = f"{entry_label_text}RedOffice{subdir}"
                #self.make_appPool(variable_name)
                #self.make_app(extract_path, subdir, variable_name)
                print(pool)
    ...
    def select_site(self):
        ...
        try:
            zip_file_path = filedialog.askdirectory(title="Select Site Directory")  # Use askdirectory to select a directory
            self.zip_file_path = zip_file_path  # Store the zip_file_path

            self.selected_zip_label.config(text=zip_file_path)  # Update the label text with the selected directory path
            
            # Remove existing checkboxes (if any)
            for widget in self.frame.winfo_children():
                if isinstance(widget, tk.Checkbutton):
                    widget.destroy()

            # Create checkboxes dynamically based on subdirectories
            subdirectories = [d for d in os.listdir(zip_file_path) if os.path.isdir(os.path.join(zip_file_path, d))]
            for i, subdir in enumerate(subdirectories):
                var = tk.BooleanVar()
                self.checkbox_vars.append(var)  # Store the BooleanVar in the list

                checkbox = tk.Checkbutton(self.frame, text=subdir, variable=var)
                checkbox.grid(row=i + 3, column=0, padx=10, pady=5, sticky="w")

        except Exception as e:
            messagebox.showerror("Error", f"An error occurred: {str(e)}")


    def locate_appPpool(self, envi):
        # Define the appcmd commands
        print(envi)
        apppool_command = [self.appcmd_path, 'list', 'app', f'/text:applicationPool']
        physical_path_command = [self.appcmd_path, 'list', 'app', self.end]
        mapping = {}
        source_folder_path = envi#os.path.normpath(self.zip_file_path)        
    ...
    def stop_appPool(self, apppool):
        try:
            # Construct the command to start the website with /apppool.name:
            command = [self.appcmd_path, 'stop', 'apppool', f'/name:{apppool}']
